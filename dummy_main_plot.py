...
from dummy_run_plot import Normal_run
from datetime import datetime


ccbdl.utils.logging.del_logger(source=__file__)

config_path = os.path.join(os.getcwd(), "dummy_config_plot.yaml")
config = ConfigurationLoader().read_config("dummy_config_plot.yaml")

# Get Configurations
network_config_nlrl = config["network_nlrl"]
network_config_linear = config["network_linear"]
...
def generate_train_folder(name: str = " ", generate: bool = False, location: str = False):
    ...
    if not location:
        location = os.getcwd()
    folder = os.path.join(location, "00_Runs_Plot", time_str + name)
    if generate:
