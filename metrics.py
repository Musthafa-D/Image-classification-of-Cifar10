...
class Metrics:
    ...
    def compute_metrics(self, method_name, method, method_map, inputs, labels):
        ...
        infidelity_score = infidelity(self.model, my_perturb_func, inputs, method_map, target=labels)
        
        if method_name == "Occlusion":
            sensitivity_score = sensitivity_max(method.attribute, inputs, target=labels, sliding_window_shapes=(3, 5, 5), strides=(3, 2, 2))
        else:
            sensitivity_score = sensitivity_max(method.attribute, inputs, target=labels)
    ...
    def calculations(self):
        ...
        for i, data in enumerate(self.test_data):
            ...
            attr, attr_maps = attributions(self.model, inputs, labels), attribution_maps(self.model, inputs, labels)
